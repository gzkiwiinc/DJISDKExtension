...
extension DJIGimbal {
    ...
    public func rotate(pitch: Double?, roll: Double?, yaw: Double?, time: TimeInterval = 0.0, mode: DJIGimbalRotationMode = .absoluteAngle, ignore: Bool = true) -> Promise<Void> {
        return Promise { seal in
            let rotation = DJIGimbalRotation(pitchValue: pitch.toNSNumber, rollValue: roll.toNSNumber, yawValue: yaw.toNSNumber, time: time, mode: mode, ignore: ignore)
