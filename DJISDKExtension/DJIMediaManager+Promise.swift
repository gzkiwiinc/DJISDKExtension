...
extension DJIMediaManager {
    ...
    public func fetchMediaFileListSnapshot(of storageLocation: DJICameraStorageLocation) -> Promise<[DJIMediaFile]> {
    ...
    public func fetchMediaContent(mediaFile: DJIMediaFile, mediaContentType: DJIFetchMediaTaskContent) -> Promise<DJIMediaFile> {
        return Promise { seal in
            ...
            let fetchTask = DJIFetchMediaTask(file: mediaFile, content: mediaContentType) { (mediaFile, fetchMediaTaskContent, error) in
    ...
    public func delete(mediaFiles: [DJIMediaFile]) -> Promise<Void> {
        return Promise { seal in
            delete(mediaFiles) { (deleteFailFiles, error) in
                if let _ = error {
                    seal.reject(DJIFileManageError.deleteFileFail(deleteFailFiles))
                } else {
                    seal.fulfill(())
                }
            }
